...
Run the names that usually appear in these lists through those questions and most of them leave the room. mem0, Letta, Zep and MemPalace are session memory: what happened, per user or per agent, in a vector store or a database. Aider, Mastra, Dify and Langflow are a coding agent and three frameworks with memory as a feature. projectmemory.app and Storyflow remember where a project stands, for people who don't open a terminal. projectmem keeps a gitignored event log and commits a digest of it, a session layer with a repo-native summary. What is left is two repo-native tools that hold rationale, the memory built into the agent most people use, and a server-backed project-memory implementation. Those four are the table.
